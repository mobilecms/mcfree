...
class User_Controller extends Controller {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ( ! $this->error) {
                ...
                if ($this->config['user']['email_confirmation'] == 1) {
                    ...
                    $msg2 = file_get_contents(ROOT .'data_files/email_templates/reg_confirmation.tpl');

                    $msg2 = str_replace('{SYSTEM_TITLE}', $this->config['system']['system_title'], $msg2);
					$msg2 = str_replace('{LINK}', a_url('user/email_confirm', 'pin_code='. $pin_code), $msg2);
					$msg2 = str_replace('{PIN_CODE_TIME}', $this->config['system']['pin_code_time'], $msg2);

                    $this->email2 = new CI_Email;

                    $this->email2->from($this->config['system']['system_email']);
                    $this->email2->to($_POST['email']);

                    $this->email2->subject('Подтверждение E-mail на '. $this->config['system']['system_title']);
                    $this->email2->message($msg2);
	...
	*/
	...
	/**
	 * Модерация пользователей, вывод уведомлений
	 */
	public function action_moderate() {
            if ($this->user['account'] == 'active' || $this->config['user']['user_moderate'] == 0 || ! $_GET['type'])
                a_error();

            switch($_GET['type']) {
                case 'moderate':
                    a_error('Ваш аккаунт находится на модерации у администрации сайта. После прохождения модерации Вы получите уведомление на E-mail.');
                break;

                case 'block':
                    a_error('Ваш аккаунт не прошел модерацию и был заблокирован.');
                break;
