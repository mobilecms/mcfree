...
class Downloads_Controller extends Controller {
	...
	*/
	public function action_list_files() {
		...
		$pg_conf['base_url'] = a_url('downloads/list_files', $http_query .'&amp;start=');
	...
	*/
	public function action_search_form() {
		...
		if(isset($_GET['send'])) {
			...
			if(!$this->error) {
				header('Location: '. a_url('downloads/list_files', 'action=search&search_word='. urlencode($_GET['search_word']) .'&directory_id='. $directory_id, true));
				exit;
			}
		}
		if(!isset($_GET['send']) OR $this->error) {
			$this->tpl->assign(array(
				'error' => $this->error,
